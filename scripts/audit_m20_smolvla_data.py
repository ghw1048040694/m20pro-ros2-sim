...
def inspect_episode(path: Path, root: Path, args: argparse.Namespace) -> dict:
    ...
    with h5py.File(path, "r") as h5:
        ...
        train_eligible = (success and command_ok or stable_search) and not holdout
...
def main() -> None:
    ...
    tasks = Counter(episode.get("task", "") for episode in eligible)
    colors = sorted(
        {
            episode["target_color"]
            for episode in eligible
            if episode.get("target_color") not in (None, "none", "")
        }
    )
    scene_ids = sorted(
        {str(episode["scene_id"]) for episode in eligible if episode.get("scene_id") is not None}
    )
    jump_episodes = [
        episode
        for episode in eligible
    ...
    obstacle_lidar_episodes = [
        episode for episode in eligible if episode.get("obstacle_height_m") is not None
    ]
    timestamp_aligned = [
        episode
        for episode in eligible
    ...
    gates = {
        ...
        "minimum_object_categories_12": len(colors) >= 12,
        "minimum_instruction_templates_24": len(tasks) >= 24,
        "all_eligible_episodes_timestamp_aligned": len(timestamp_aligned) == len(eligible),
